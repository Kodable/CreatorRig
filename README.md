...
| `purchase` | CW-01.9 | One sandbox auto-renewable subscription through StoreKit 2 (`@capgo/native-purchases`), then restore and read the entitlement, shell only | transaction returned with the subscription active |
...
- **Purchase**: `@capgo/native-purchases` (StoreKit 2, no account). App Store Connect needs an app record for the bundle id with one auto-renewable subscription `com.surfscore.kodable.creatorrig.monthly` in a subscription group, and a sandbox tester signed in on the device (a sandbox month renews every 5 minutes). Subscribe, then "Restore and check entitlement" reads the state back the way the product would on launch. The report carries the transaction id, subscription state, expiration and receipt sizes; the console prints `RIG_RECEIPT` with the JWS.
