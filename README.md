...
- `audio` creates the AudioContext on the first tap and plays a 250 ms beep on every tap. It resumes the context and beeps again when the tab returns to the foreground and after Safari reports an `interrupted` state (a call, Siri, a timer). "Played" means the context is running, the oscillator's `ended` fired and the audio clock advanced. The report carries the on-screen log.
