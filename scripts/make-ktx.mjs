...
for (const t of TEXTURES) {
  ...
  execFileSync(ASTCENC, ['-cl', t.png, `${t.out}-astc.ktx`, '6x6', '-medium', '-silent']);
  console.log(`  ${t.out}-astc.ktx ${mb(`${t.out}-astc.ktx`)} (ASTC 6x6, iPad and iPhone)`);

  execFileSync(ETCTOOL, [t.png, '-format', 'RGBA8', '-effort', '40', '-output', `${t.out}-etc2.ktx`], { stdio: 'ignore' });
  console.log(`  ${t.out}-etc2.ktx ${mb(`${t.out}-etc2.ktx`)} (ETC2 RGBA8, Chromebook)`);

  // KTX 1 images are stored bottom row first (OpenGL order), so flip the PNG rows before
  // compression. Range fit is the fast libsquish mode; quality is enough for a memory and rendering check.
